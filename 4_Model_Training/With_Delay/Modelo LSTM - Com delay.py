    ...
    for SEQ_LEN in RANGE_SEQ_LEN:
        for NUM_LAYERS in RANGE_NUM_LAYERS:
            for igr_i in data['igr'].unique():
                ...
                # ==== Function to create windows with hospitalization delay ====
                def create_sequences(var1, target, seq_len, output_size, delay):
                    x1s, ys = [], []
                    for i in range(len(target) - seq_len - output_size + 1):
                        # --- Hospitalization with delay ---
                        past_intern = var1[i:i + seq_len - delay]  # until t-delay
                        intern_padded = np.pad(past_intern, (delay, 0), constant_values=0)
                ...
                x1_train, y_train = create_sequences(
                ...
                x1_test, y_test = create_sequences(
                ...
                #print(f"\nnMetrics: {metrics_comparison}")

# COMMAND ----------

# Variáveis de clima por microrregiao
    ...
    for SEQ_LEN in RANGE_SEQ_LEN:
        for NUM_LAYERS in RANGE_NUM_LAYERS:
            for igr_i in data['igr'].unique():
                ...
                # ==== Function to create windows with hospitalization delay ====
                def create_sequences_duplas(var1, var2, var3, var4, var5, target, seq_len, output_size, delay):
    ...
    for SEQ_LEN in RANGE_SEQ_LEN:
        for NUM_LAYERS in RANGE_NUM_LAYERS:
            for igr_i in data['igr'].unique():
                ...
                # ==== Function to create windows with hospitalization delay ====
                def create_sequences_duplas(var1, var2, target, seq_len, output_size, delay):
                    x1s, x2s, ys = [], [], []
                    for i in range(len(target) - seq_len - output_size + 1):
                        # --- Hospitalization with delay ---
                        past_intern = var1[i:i + seq_len - delay]  # até t-delay
...
try:
    ...
    df_metrics_final.to_excel(f"result_lstm_with_delay_{seed_number}_70.xlsx", index=False)
