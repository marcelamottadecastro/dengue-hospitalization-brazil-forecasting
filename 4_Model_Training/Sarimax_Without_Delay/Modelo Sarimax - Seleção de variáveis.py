...
for igr_i in data['igr'].unique():
    ...
    try:
        ...
        y_train, y_test = y_scaled.iloc[:split], y_scaled.iloc[split:]
...
try:
    ...
    df_final_metrics.drop(columns=['y_train', 'y_test', 'pred'], errors='ignore').to_excel(f"best_sarimax_without_delay_{seed_number}.xlsx", index=False)
