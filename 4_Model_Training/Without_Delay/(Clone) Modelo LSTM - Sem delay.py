    ...
    for SEQ_LEN in RANGE_SEQ_LEN:
        for NUM_LAYERS in RANGE_NUM_LAYERS:
            for igr_i in data['igr'].unique():
                ...
                x3_seq = data_micro[var_exogenicas[1]].values.astype(np.float32)
                x4_seq = data_micro[var_exogenicas[2]].values.astype(np.float32)
                x5_seq = data_micro[var_exogenicas[3]].values.astype(np.float32)
    ...
    for SEQ_LEN in RANGE_SEQ_LEN:
        for NUM_LAYERS in RANGE_NUM_LAYERS:
            for igr_i in data['igr'].unique():
                ...
                x4_seq = data_micro[var_exogenicas[2]].values.astype(np.float32)
                x5_seq = data_micro[var_exogenicas[3]].values.astype(np.float32)
                x6_seq = data_micro[var_exogenicas[4]].values.astype(np.float32)
...
try:
    ...
    df_metrics_final.to_excel(f"result_lstm_without_delay_{seed_number}.xlsx", index=False)
...
pd.read_excel(f"result_lstm_without_delay_{seed_number}.xlsx")
...
display(df_metrics_final[['igr', 'network_architecture', 'RMSE']].groupby(['igr', 'network_architecture']).min().reset_index())

# COMMAND ----------

df_metrics_final[['igr', 'network_architecture', 'RMSE']].groupby(['igr', 'network_architecture']).min()
...
display(best_each_type[['igr', 'network_architecture', 'features', 'RMSE', 'epoch', 'n_layers', 'hidden_size', 'seq_len']])
...
df = pd.read_excel(f"amostra_result_lstm_without_delay_7809.xlsx")
df
...
def plot_true_vs_pred(true, pred, title='True vs Pred Test'):
    plt.figure(figsize=(10, 5))
    plt.plot(true, label='True Test')
    plt.plot(pred, label='Pred Test')
...
plot_true_vs_pred(df['true_test'][0][0], df['pred_test'][0][0])

# COMMAND ----------

time_prediction = 4

plot_true_vs_pred(df['true_test'][0][time_prediction], df['pred_test'][0][time_prediction])